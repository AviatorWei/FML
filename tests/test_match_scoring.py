...
def test_own_goal_credits_own_team():
    ...
    rules = default_rules()
    calc = ValidGoalCalculator(rules)
    events = [Ev(1, 10, RealEventType.OWN_GOAL)]
    score = calc.score(
        fixture_id=1, gameweek_id=1,
...
def test_shootout_excluded():
    ...
    calc = ValidGoalCalculator(rules)
    events = [
        Ev(1, 10, RealEventType.GOAL, is_shootout=True),
        Ev(1, 10, RealEventType.GOAL, is_shootout=False),
    ]
    score = calc.score(
        fixture_id=1, gameweek_id=1,
        home_starter_ids=[10], away_starter_ids=[20],
...
def test_player_not_a_starter_ignored():
    rules = default_rules()
    calc = ValidGoalCalculator(rules)
    events = [Ev(1, 99, RealEventType.GOAL)]
    score = calc.score(
        fixture_id=1, gameweek_id=1,
        home_starter_ids=[10], away_starter_ids=[20],
        events=events,
